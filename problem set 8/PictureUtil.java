...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight(Picture pic) {
        ...
        int[][] gray = pic.getGrayLevels();
        int[][] flipped = new int[gray.length][gray[0].length];
        for (int row = 0; row < gray.length; ++row) {
            for (int col = 0; col < gray[0].length; ++col) {
                flipped[row][gray[0].length - col - 1] = gray[row][col];
            }
        }
        return new Picture(flipped);
    }
    ...
    public static Picture grayAndRotate90( Picture pic)
    {
        ...
        int[][] gray = pic.getGrayLevels();
        int[][] rotated = new int[gray[0].length][gray.length];
        for (int row = 0; row < gray.length; ++row) {
            for (int col = 0; col < gray[0].length; ++col) {
                rotated[col][gray.length - row - 1] = gray[row][col];
            }
        }
